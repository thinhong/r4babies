...
::: center-text
[{{< fa link >}} Presentation](https://drthinhong.com/r4babies-slides/prepare.html){.btn .btn-outline-secondary role="button"}
:::
...
{{< downloadthis data/gdp-capita.xlsx dname="gdp-capita" label="gdp-capita.xlsx" type=light >}} {{< downloadthis data/life-exp.csv dname="life-exp" label="life-exp.csv" type=light >}} {{< downloadthis data/linelist-raw.dta dname="linelist-raw" label="linelist-raw.dta" type=light >}}

-   Cài đặt các R packages này:
...
Trên thanh công cụ của Finder, chọn `Show Path Bar`

![](img/prepare/path-mac-1.png)

Click chuột phải vào file/folder trên Path Bar, chọn `Copy "..." as Pathname`

![](img/prepare/path-mac-2.png)

### Absolute path và relative path {#sec-abs-rel-path}
...
Từ bây giờ, mỗi lần muốn phân tích cho dự án `/test` này, chúng ta mở file `test.Rproj` bằng RStudio. Tất cả các file code trong folder `/analysis` có thể đọc file trong folder `/data` bằng cách dùng relative path. Những data quan trọng không thể copy vào chung trong project thì dùng absolute path (xem lại phần nói về path ở @sec-abs-rel-path). Ví dụ:
...
-   `file`: path của file data đầu vào, có thể dùng absolute path hoặc relative path (xem @sec-abs-rel-path).
...
Dùng lệnh `read_excel()` trong package `readxl` để đọc các file excel (đuôi `.xls` hay `.xlsx`) . Xem lại cách cài đặt và sử dụng R packages tại @sec-r-pkgs.
