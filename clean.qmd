...
Để có đầy đủ thông tin bệnh nhân và bệnh viện, ta cần ghép 2 bảng với nhau. Cột để xác định các hàng cần được nối là cột `hospital_id` bên bảng bệnh viện và cột `id` bên bảng bệnh nhân.
...
**Ví dụ:** Thực hiện nối bảng trên dữ liệu vaccine để kiểm tra và loại bỏ các hàng quận/huyện, phường/xã không hợp lệ
...
Đọc sheet thứ 2 của file `gdp-capita.xlsx` và nối với bảng dữ liệu đã được chỉnh sửa
