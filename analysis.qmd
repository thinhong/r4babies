...
{{< downloadthis data/linelisting.rds dname="linelisting" label="linelisting.rds" type=light >}}

{{< downloadthis data/cleaned_vacdata.rds dname="cleaned_vacdata" label="cleaned_vacdata.rds" type=light >}}
...
**Thay đổi cách hiển thị biến qua tham số `label`**
...
both_outbreak <- linelist %>% 
  ...
  modify_header(all_stat_cols() ~ "**{level}**", label = "**District**") 
...
tbl_stack(list(tbl_first_outbreak, tbl_second_outbreak, both_outbreak), group_header = c("1st outbreak", "2nd outbreak", "Both outbreak")) %>% 
...
tbl_stack(list(first_outbreak, second_outbreak, both_outbreak), 
          group_header = c("1st outbreak", "2nd outbreak", "both outbreak")) %>% 
...
tbl_first_outbreak <- linelist %>%
  filter(outbreak == "1st outbreak") %>%
  tbl_summary(include = c(sex, outcome), by = sex) %>%
  # điều chỉnh header để các bảng có chung header
  modify_header(all_stat_cols() ~ "**{level}**") %>% 
  add_p(test = outcome ~ "chisq.test")
...
both_outbreak <- linelist %>% 
  tbl_summary(
    include = c(sex, outbreak, outcome),
    by = c(sex)
  ) %>% 
  # điều chỉnh header để các bảng có chung header
  modify_header(all_stat_cols() ~ "**{level}**") %>% 
...
# ----- Bảng kết quả cho đợt dịch đầu tiên ---- 
tbl_first_outbreak <- linelist %>%
  filter(outbreak == "1st outbreak") %>%
  tbl_summary(include = c(sex, outcome), by = sex) %>%
  modify_header(all_stat_cols() ~ "**{level}**") %>% 
...
tbl_stack(list(tbl_first_outbreak, tbl_second_outbreak, both_outbreak), 
          group_header = c("1st outbreak", "2nd outbreak", "both_outbreak"))
```
...
Đọc dữ liệu tiêm chủng đã được làm sạch

```{r}
vaccdata <- readRDS("data/cleaned_vacdata.rds")
```

Và tạo cột tuổi tiêm

```{r}
vaccdata <- vaccdata %>% mutate(
  tuoi = difftime(ngaytiem, ngaysinh, units = "days")
)
```
...
-   Hiển thị thống kê cho các biến `tuoi`, `ngaysinh`, `ngaytiem`, `khangnguyen` cho từng nhóm giới tính (`gioitinh`)
...
vaccdata %>% tbl_summary(
  ...
  include = c(gioitinh, ngaysinh, ngaytiem, tuoi, khangnguyen),
...
Tạo bảng thống kê độ tuổi tiêm mũi ho gà đầu tiên theo quận huyện

```{r}
vaccdata %>% 
  filter(khangnguyen == "hg_1") %>% tbl_summary(
    # chọn biến được hiển thị trên bảng thống kê
    include = c(tuoi, huyen),
    # thay đổi tên được hiển thị
    label = list(
        tuoi ~ "Ngày tuổi",
        huyen ~ "Quận/Huyện"
...
-   Tính tỷ lệ tiêm chủng bằng `summary()`

-   Hiển thị kết quả bẳng `gt()`
